...
export class ExportCoordinator {
  ...
  private async persistDefaults(session: PreparedCompileSession): Promise<void> { const settings = this.settings(); settings.defaultManuscriptFolder = session.request.manuscriptRoot; settings.defaultExportFolder = session.request.exportFolder; settings.defaultStructurePreset = session.request.structurePreset; settings.defaultDocxStyle = session.request.docxPreset; settings.defaultExportFormat = session.request.outputFormat; await this.saveSettings(); }
