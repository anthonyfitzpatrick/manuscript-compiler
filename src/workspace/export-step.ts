...
export function renderExportStep(container: HTMLElement, controller: CompileWorkspaceController, actions: ExportStepActions): void {
  ...
  new Setting(container).setName("Book title").addText((text) => text.setValue(state.request.custom?.variables?.BookTitle || preview.book.title).onChange((value) => { controller.setVariable("BookTitle", value); actions.changed(); }));
  new Setting(container).setName("Author").addText((text) => text.setValue(state.request.custom?.variables?.Author || "").onChange((value) => { controller.setVariable("Author", value); actions.changed(); }));
  ...
  new Setting(container).setName("Save a copy in the vault").setDesc("The DOCX is saved here so it remains available with your project.").addText((text) => text.setValue(state.request.exportFolder).setPlaceholder("Manuscript Exports").onChange((value) => { controller.setOutput(value, state.request.outputFilename); actions.changed(); }));
