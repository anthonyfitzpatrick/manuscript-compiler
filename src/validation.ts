...
export interface ValidationResult { book: Book; issues: CompileWarning[]; docxEngine: "built-in"; durationMs: number; }
export class ManuscriptValidationService {
  ...
  async validate(scan: ScannedBook, profile: CompileProfile): Promise<ValidationResult> {
    ...
    return { book, issues: this.deduplicate(issues), docxEngine: "built-in", durationMs: performance.now() - started };
  }
