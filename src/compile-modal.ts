...
export class SimpleCompileModal extends Modal {
  ...
  constructor(app: App, private readonly plugin: ManuscriptCompilerPlugin) {
    ...
    const formatting: DocxFormatting = { font: "Times New Roman", fontSize: 12, lineSpacing: 2, firstLineIndent: 0.5, pageSize: "letter", chapterPageBreak: true, titlePage: settings.includeTitlePageByDefault };
  ...
  private render(): void {
    ...
    else if (state.step === "formatting") renderFormattingStep(body, this.controller, this.plugin.settings.includeTableOfContentsByDefault, (value) => { this.plugin.settings.includeTableOfContentsByDefault = value; });
  ...
  private async export(): Promise<void> { const promise = this.controller.export(); this.controller.detachExport(); this.close(); const success = await promise; if (!success && this.controller.state.error) new Notice(this.controller.state.error.message, 8000); }
