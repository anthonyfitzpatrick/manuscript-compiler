...
import { pathExists } from "./pandoc";
import { getObsidianVersion, openExternalVaultFile } from "./platform-compat";
...
export default class ManuscriptCompilerPlugin extends Plugin {
  ...
  async detectPandoc() { return { available: true, version: "Built-in DOCX", explanation: "Pandoc is not required." }; }
  ...
  private async generateDiagnostics(): Promise<void> { try { const report = new DiagnosticsReportGenerator().generate({ pluginVersion: this.manifest.version, obsidianVersion: getObsidianVersion(), operatingSystem: navigator.userAgent, profile: this.getActiveProfile(), settings: this.settings, pandoc: { available: true, version: "Built-in DOCX" } }); new DiagnosticsReportModal(this.app, report, () => this.saveDiagnostics(report)).open(); } catch (error) { showError(error); } }
  ...
  async compileFolder(folder: TFolder): Promise<void> {
    ...
    try {
      ...
      const pandoc = { available: true, version: "Built-in DOCX", explanation: "DOCX is generated locally without Pandoc." }; pandocVersion = pandoc.version;
      ...
      progress = new CompilationProgressModal(this.app, () => controller.abort()); progress.open(); const request = (outputPath: string): ExportRequest => ({ book, profile, markdown: result!.markdown, outputPath, variables, pandoc, keepTemporaryMarkdown: profile.keepIntermediateMarkdown || this.settings.keepTemporaryMarkdown, signal: controller.signal, onCommit: () => progress.lock("Finalizing export atomically…") });
