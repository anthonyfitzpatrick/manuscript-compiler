...
import { openExternalVaultFile } from "./platform-compat";
...
import { CompilePreviewModal, CompileReportModal, ConfirmOverwriteModal, FolderSuggestModal, ManuscriptCompilerSettingTab, showError, ValidationReportModal } from "./ui";
...
export default class ManuscriptCompilerPlugin extends Plugin {
  ...
  private async compileFolder(folder: TFolder): Promise<void> {
    const started = Date.now(); const timestamp = new Date(); const profile = this.getActiveProfile(); const outputFiles: string[] = []; let result: CompileResult | undefined; let pandocVersion: string | undefined; let diagnostics = ""; let scanDurationMs = 0; let filterDurationMs = 0; let generationDurationMs = 0; let exportDurationMs = 0;
    try {
      new Notice(`Preparing “${folder.name}” with profile “${profile.name}”…`);
      const scanStarted = performance.now(); const scan = new VaultScanner().scan(folder); scanDurationMs = performance.now() - scanStarted; const compiler = new ManuscriptCompiler(this.app.vault); const book = await compiler.buildModel(scan, profile); filterDurationMs = compiler.timings.filterDurationMs; const markdownExporter = new MarkdownExporter(this.app.vault);
      ...
      const paths = this.targetPaths(profile.exportTarget, markdownPath, docxPath); const primaryPath = paths[0] ?? markdownPath; result = compiler.compile(book, profile, primaryPath, this.settings.readingWordsPerMinute, timestamp); generationDurationMs = compiler.timings.generationDurationMs;
      ...
      const exportStarted = performance.now(); if (profile.exportTarget !== "docx") { const exported = await markdownExporter.export(request(markdownPath)); outputFiles.push(exported.path); }
      ...
      exportDurationMs = performance.now() - exportStarted; const report: CompileResult = { ...result, issues: visibleIssues, warnings: visibleIssues.map((issue) => issue.message) }; const fullSuccess = profile.exportTarget === "markdown" || docxReady; await this.recordExport(fullSuccess, timestamp, started, profile.name, folder.path, profile.exportTarget, outputFiles, report, pandocVersion, fullSuccess ? diagnostics : "DOCX output was skipped because Pandoc or its configured inputs were unavailable.", { scanDurationMs, filterDurationMs, generationDurationMs, exportDurationMs }); new CompileReportModal(this.app, outputFiles.join(", "), report, this.settings.showStatistics).open();
    } catch (error) {
      ...
      await this.recordExport(false, timestamp, started, profile.name, folder.path, profile.exportTarget, outputFiles, result, pandocVersion, diagnostics, { scanDurationMs, filterDurationMs, generationDurationMs, exportDurationMs }); showError(new Error(this.friendlyError(error)));
    }
  }
