...
function safeLeaf(value: string): string { let leaf = replaceUnsafeFilenameCharacters(value.replace(/^.*[\\/]/, "")).trim().replace(/[. ]+$/g, ""); if (/^(?:con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(leaf)) leaf = `_${leaf}`; return limitFilename(leaf || "Manuscript"); }
...
function limitFilename(value: string): string { const match = /^(.*?)(\.[A-Za-z0-9]{1,12})$/.exec(value); const extension = match?.[2] ?? ""; const stem = match?.[1] ?? value; const maximum = 200 - new TextEncoder().encode(extension).length; let result = ""; let bytes = 0; const encoder = new TextEncoder(); for (const character of stem) { const size = encoder.encode(character).length; if (bytes + size > maximum) break; result += character; bytes += size; } return `${result.replace(/[. ]+$/g, "") || "Manuscript"}${extension}`; }
