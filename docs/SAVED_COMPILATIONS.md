...
| Observed source state | Supports reconciliation and truthful freshness display | Compact path/structure/fingerprint/stat evidence and safe last-export facts | Prose, whole YAML/frontmatter, absolute paths |
...
| Cross-format presentation | title/author override, scene separator/custom scene break, table of contents where applicable, Part/Chapter display, scene-title/body-heading choice. |
...
| Duplicate | A management action that prompts for a name and copies the persisted recipe into a fresh ID without switching to it. | Copies persisted author intent/output values; does not copy an active unsaved overlay or export freshness/history. |
...
Export may proceed with dirty recipe edits if the current manuscript has been prepared and source/input freshness checks pass. The result is truthfully an export of unsaved choices; the UI shows that fact and leaves the recipe dirty. Closing, switching, creating a new compilation, or re-associating a root prompts only for dirty recipe changes: **Save changes**, **Discard changes**, or **Cancel**. “Discard” restores the last persisted recipe then reconciles it against the current vault; it never reverts manuscript notes.
...
| Export out of date | Last successful browser dispatch used a different observed source/input/recipe state. | Informational; current export is not blocked by history alone. |
...
## Fingerprints and export freshness

The existing `PreparedCompileSession` already carries an included-source content fingerprint and a semantic input signature. `ExportCoordinator` recalculates both before delivery; source hashing catches equal-size edits with unchanged timestamps. That machinery remains the authoritative per-export freshness guard and must not be bypassed.

Saved Compilations may later retain compact hashes/signatures only for four conceptual purposes:
...
Allowed persisted categories are relative root/item paths, display names, roles, ordering, output/cleaning settings, non-prose fingerprint evidence, and timestamps/safe export facts. Relative paths and names may still be sensitive. Diagnostics may report counts (Saved Compilations, statuses, malformed entries, schema state) but must redact names, paths, filenames, fingerprints, recipes, metadata values, and export destinations. Support export must never dump the saved-compilation database.
...
| 3. Edit prose only | Fingerprint/stat comparison shows source content changed, no mapping conflict. | Recipe unchanged; refresh prepares new prose; prior export becomes out of date. |
...
| 10. Duplicate Vellum Master as Agent Submission | Duplicate action asks for name and loads new recipe. | New ID/timestamps, copied recipe/output choices, no copied export freshness/history. |
...
| 14. Export with unsaved recipe changes | Header says unsaved changes; current session must still pass freshness checks. | Export is allowed; recipe remains dirty and last successful export can be recorded against current unsaved state only as non-persisted session fact until saved. |
...
13. Export freshness is informational about browser dispatch, never knowledge of external persistence.
