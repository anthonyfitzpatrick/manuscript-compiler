...
const benchmarkPlan = allMarkdown.map((item, index) => ({ path: item.path, parentPath: item.path.slice(0, item.path.lastIndexOf("/")), name: item.basename, kind: "note" as const, role: "scene" as const, detectedRole: "scene" as const, included: true, order: index }));
const savedBenchmark = { root: { path: "Book" }, recipe: { overrides: [], manualOrders: [], structurePreset: "novel", includeFrontMatter: true, includeBackMatter: true, includeSceneTitles: true, cleaning: {}, metadataFilters: [], useParts: true, chapterSource: "folders", orderingMethod: "filename", metadataOrdering: false, partHeadingTemplate: "{title}", chapterHeadingTemplate: "{title}", blankLinesBetweenSections: 1, blankLinesBetweenChapters: 1 }, output: { format: "docx", filename: "Benchmark.docx", docxPreset: "vellum", title: "Benchmark", author: "", tableOfContents: false, sceneSeparator: "#", partDisplay: "word-title", chapterDisplay: "word-title", titlePage: false }, observedSource: { references: benchmarkPlan.map((item) => ({ path: item.path.slice(5), parentPath: item.parentPath.slice(5) || "_root", name: item.name, kind: item.kind, expectedRole: item.role })) } } as never;
const savedUnchangedStarted = performance.now(); reconcileSavedCompilation({ compilation: savedBenchmark, rootPath: "Book", plan: benchmarkPlan }); const savedUnchangedMs = performance.now() - savedUnchangedStarted;
const savedChangedStarted = performance.now(); reconcileSavedCompilation({ compilation: savedBenchmark, rootPath: "Book", plan: benchmarkPlan.slice(1) }); const savedChangedMs = performance.now() - savedChangedStarted;
...
process.stdout.write(`Large-manuscript benchmark: ${statistics.chapterCount} chapters, ${statistics.sceneCount} scenes, ${statistics.totalWordCount.toLocaleString()} words in ${Math.round(duration)} ms. New preparation parse/clean/Book ${Math.round(parseMs)} ms; Saved unchanged reconciliation ${Math.round(savedUnchangedMs)} ms (one preparation route); Saved structural-change reconciliation ${Math.round(savedChangedMs)} ms (one preparation route). Statistics ${Math.round(statisticsMs)} ms; Markdown ${Math.round(markdownMs)} ms; semantic projection ${Math.round(semanticMs)} ms. Exports from the same Book: ${formatResults.map((item) => `${item.format.toUpperCase()} indent ${item.indentation ? "on" : "off"} ${Math.round(item.ms)} ms/${item.bytes.toLocaleString()} bytes`).join("; ")}.${memory === undefined ? "" : ` Observed heap after generation: ${memory} MiB.`}\n`);
