...
test("settings end with an accessible responsive Support & Links panel", async () => {
  ...
  assert.match(ui, /Bug reporting portal coming soon\./); assert.match(ui, /Feature request portal coming soon\./); assert.match(ui, /new ButtonComponent\(actions\)/); assert.match(ui, /setIcon\(action\.icon\)/); assert.match(ui, /buyMeACoffeeArtwork/); assert.match(ui, /manuscript-support-bmc-icon/); assert.match(ui, /setAttribute\("aria-label", action\.label\)/); assert.match(ui, /button\.buttonEl\.win\.open\(action\.url, "_blank", "noopener,noreferrer"\)/);
  assert.match(css, /\.manuscript-support-panel \{[^}]*width: 100%;[^}]*var\(--background-modifier-border\)[^}]*var\(--background-secondary\)/s); assert.match(css, /\.manuscript-support-actions \{[^}]*display: flex;[^}]*flex-wrap: wrap;/s); assert.match(css, /\.manuscript-support-button \{[^}]*flex: 0 1 auto;[^}]*min-height: 60px;[^}]*box-shadow: var\(--shadow-s\)/s); assert.match(css, /\.manuscript-support-button-coffee \{[^}]*border-color: var\(--interactive-accent\)/s); assert.match(css, /\.manuscript-support-bmc-icon \{[^}]*overflow: hidden;/s); assert.match(css, /\.manuscript-support-button:focus-visible/); assert.match(css, /@media \(max-width: 700px\)[\s\S]*\.manuscript-support-button \{ flex-basis: 100%; \}/);
});
