...
import { ExportCoordinator, friendlyExportError } from "../src/export-coordinator";
...
function renderedText(format: ExportFormat, bytes: Uint8Array): string {
  if (format === "docx") return strFromU8(unzipSync(bytes)["word/document.xml"]);
  if (format === "odt") return strFromU8(unzipSync(bytes)["content.xml"]);
  if (format === "epub") return Object.entries(unzipSync(bytes)).filter(([name]) => name.endsWith(".xhtml")).map(([, value]) => strFromU8(value)).join("\n");
...
test("every native export format omits structural Scene titles from the prepared manuscript", "common", async () => {
  const sceneTitles = ["Scene 1 - Opening", "Scene 2 - The Barn"];
  const bodies = ["Östersund was silent beneath the winter stars", "The barn door opened on darkness"];
  for (const format of EXPORT_FORMATS) {
    const text = renderedText(format, (await output(format)).bytes);
    sceneTitles.forEach((title) => assert.equal(text.includes(title), false, `${format} must not render ${title}`));
    bodies.forEach((body) => assert.equal(text.includes(body), true, `${format} must retain ${body}`));
    assert.equal(text.includes("The Silence of Östersund"), true, `${format} must retain the Chapter title`);
    assert.equal(text.includes("The Silence Breaks"), true, `${format} must retain the Part title`);
  }
});

test("every native export format preserves the parser-cleaned punctuation-mismatch Scene body", "common", async () => {
  const structuralTitle = "Scene 4 – The Choice, and the Collision";
  const sourceHeading = "Scene 4 - The Choice and the Collision";
  const body = "The moment Sophie realized what happened, it was as if every drop of blood.";
  const firstPart = session.book.parts[0]; const firstChapter = firstPart.chapters[0]; const sourceScene = firstChapter.scenes[0];
  const mismatchBook = { ...session.book, parts: [{ ...firstPart, chapters: [{ ...firstChapter, scenes: [{ ...sourceScene, title: structuralTitle, content: body }, ...firstChapter.scenes.slice(1)] }, ...firstPart.chapters.slice(1)] }, ...session.book.parts.slice(1)] };
  const mismatchSession = { ...session, book: mismatchBook }; const mismatchDocument = createSemanticDocument(mismatchBook, session.profile, options, session.statistics.totalWordCount);
  for (const format of EXPORT_FORMATS) {
    const result = await EXPORTERS[format].generate({ session: mismatchSession, document: mismatchDocument, options, filename: `mismatch.${EXPORT_FORMAT_DETAILS[format].extension}` });
    const text = renderedText(format, result.bytes);
    assert.equal(text.includes(structuralTitle), false, `${format} must not reconstruct the structural title`);
    assert.equal(text.includes(sourceHeading), false, `${format} must not render the stripped source heading`);
    assert.equal(text.includes(body), true, `${format} must retain Scene body prose`);
  }
});
...
test("portable filenames correct paths, duplicate extensions, and Windows reserved names", "common", () => {
  ...
  const bounded = exportFilename("雪".repeat(300), "docx"); assert.ok(new TextEncoder().encode(bounded).length <= 200); assert.match(bounded, /\.docx$/);
});

test("author-facing export errors redact paths and multiline technical details", "common", () => { const message = friendlyExportError(new Error("Failed at /Users/alice/Private/Book.docx\nstack secret")); assert.doesNotMatch(message, /Users|alice|Private|stack secret/); assert.match(message, /<path redacted>/); });
...
test("browser download clicks once, uses properties, removes the anchor, and revokes its URL", "common", async () => {
  ...
  const bounded = await new BrowserDownloadService(environment).download({ filename: `${"雪".repeat(300)}.md`, bytes: new Uint8Array([1]), mimeType: "text/markdown;charset=utf-8" }); assert.ok(new TextEncoder().encode(bounded.filename).length <= 200); assert.match(bounded.filename, /\.md$/);
});
...
test("Markdown is publication-ready UTF-8 with semantic structure, matter, scenes, and canonical spacing", "markdown", async () => {
  ...
  assert.match(text, /^# Warden & <Silence> 雪\n\nA & B 'Writer'/); assert.match(text, /^# (?:Copyright|Title Page)$/m); assert.match(text, /^# Part One — The Silence Breaks$/m); assert.match(text, /^## Chapter One — The Silence of Östersund$/m); assert.doesNotMatch(text, /^### Scene 1 - Opening$/m); assert.match(text, /^# Acknowledgements$/m);
